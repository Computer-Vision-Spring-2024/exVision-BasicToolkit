...
import numpy as np
from Helpers.HelperFunctions import gaussian_weight
...
def rg_canvas_clicked(self, event):
    if event.xdata is not None and event.ydata is not None:
        x = int(event.xdata)
        y = int(event.ydata)
        print(f"Clicked pixel at ({x}, {y}) with value {self.rg_input_grayscale[y, x]}")

        # Plot a dot at the clicked location
        ax = self.ui.region_growing_input_figure_canvas.figure.gca()
        ax.scatter(x, y, color="red", s=10)  # Customize the color and size as needed
...
def apply_k_means(self):
    ...
    if self.k_means_LUV:
        self.k_means_output = self.k_means_output.reshape(self.k_means_luv_input.shape)
    else:
...
def mean_shift_clusters(self, image, window_size, threshold, sigma, max_iterations=100):
    """
...
def apply_mean_shift(self):
    ...
    self.mean_shift_output = (self.mean_shift_output - self.mean_shift_output.min()) / (
